...
- 一个简单的局域网管理页面，可查看运行状态、最近事件以及开启或关闭规则；
- 通过 MiService 直连小爱音箱，不要求安装 Home Assistant；
- 摄像头断线自动恢复、规则冷却，以及规则开关和事件记录持久化。
...
- 摄像头：已验证小白智能摄像机和小白智能摄像机 2.5K 版；MVP 只要求配置一条名为
  `xiaobai` 的可用码流。
- 音箱：已验证小米小爱音箱 Play 增强版，型号 `xiaomi.wifispeaker.l05c`。
- 规则：检测到人员从画面外进入时，随机播放一条普通话欢迎语。规则默认关闭。
...
输出中必须出现 `xiaobai`。摄像头也必须已通电、开机，并在米家 App 中显示在线。
...
`MI_USER`、`MI_PASS` 和 `MI_DID` 必须与已经通过 MiService 查询的同一账号及音箱一致。
...
摄像头在线时，`camera` 和 `detector` 应最终变为 `ready`。摄像头关闭或临时断线时，接口仍
可访问，但 `status` 或 `camera` 会显示 `degraded`。
...
“人员进入欢迎”规则初始为关闭状态。在管理页面中开启规则后，先让画面连续无人至少 10 秒，
...
升级代码后重建应用，数据库、摄像头配置和 MiService Token 会保留在宿主机：
